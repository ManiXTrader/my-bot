...
export const BotAutoScanner: React.FC<BotAutoScannerProps> = ({
  ...
  const [scannedPairsCount] = useState(TRADING_PAIRS.length);
                ...
                <span
                  className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded border ${
                    isAutoRunning
                      ...
                      : 'bg-[#1E2329] text-gray-400 border-gray-700'
                  }`}
                >
                  {isAutoRunning
                    ? '● LIVE SCANNING ALL PAIRS'
                    : 'PAUSED'}
                </span>
              </div>

              <p className="text-xs text-gray-400 mt-0.5">
                Continuously scans all 20+ OTC and Live pairs for
                96%+ Amir FX Candlestick & S&R Setups
              </p>
            </div>
          </div>
        ...
        <div className="pt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
          <div className="p-3 rounded-lg bg-[#1E2329] border border-gray-800 flex items-center justify-between">
            ...
            <select
              value={minAccuracy}
              onChange={(e) =>
                setMinAccuracy(Number(e.target.value))
              }
          ...
          <div className="p-3 rounded-lg bg-[#1E2329] border border-gray-800 flex items-center justify-between">
            ...
            <span className="text-white font-mono font-bold">
              {scannedPairsCount} Live & OTC Pairs
            </span>
          </div>
      ...
      <div className="space-y-2.5">
        <div className="flex items-center justify-between px-1">
          <h3 className="font-['Rajdhani'] font-bold text-sm text-gray-300 uppercase tracking-wider flex items-center gap-2">
            ...
            Real-time Auto Generated Signal Feed
          </h3>
        ...
        {autoSignals.length === 0 ? (
          <div className="p-10 rounded-xl bg-[#161A1E] border border-gray-800 text-center text-gray-500 text-xs">
            ...
            {isAutoRunning
              ? 'Scanning all OTC & Live pairs... Next high-confidence signal incoming!'
              : 'Auto Bot is paused. Click "START 24/7 AUTO BOT" above.'}
          </div>
        ) : (
